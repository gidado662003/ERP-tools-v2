...
function page() {
    ...
    const handleBack = async () => {
        setCursorStack((prev) => {
            ...
            fetchRequests(previousCursor?.cursorId,
                previousCursor?.cursorTimeStamp);
        ...
        <div>
            ...
            <div className='grid grid-cols-5 gap-3 p-3'>
                <div onClick={() => { setStatusData("") }}>
                    ...
                    <RequestListCards label="Total" amount={listCount?.countTotal} variant='accent' icon={<FileText />} />
                </div>
                <div onClick={() => { setStatusData("pending") }}>

                    <RequestListCards label="Pending" amount={listCount?.pendingTotal} icon={<Clock />} variant='warning' />
                </div>
                <div onClick={() => { setStatusData("approved") }}>

                    <RequestListCards label="Approved" amount={listCount?.approvedTotal} variant='success' icon={<Check />} />
                </div>
                <div onClick={() => { setStatusData("rejected") }}>

                    <RequestListCards label="Rejected" amount={listCount?.rejectedTotal} variant='warning' icon={<X />} />
                </div>
                <div onClick={() => { setStatusData("outstanding") }}>

                    <RequestListCards label="Outstanding" amount={listCount?.outstandingTotal} variant='warning' icon={<X />} />
                </div>
            </div>
            <main>
                ...
                <RequestTable data={data} hasMore={hasMore} onNext={handleNext} onBack={handleBack} />
            </main>
        </div>
    )
}
