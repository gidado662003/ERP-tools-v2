...
function RequestTable({ data, hasMore, onNext, onBack }: RequestTableProps) {
    ...
    const handleSelectRequest = (request: InternalRequisition) => {
        ...
        //     // Filter out by comparing the ID property
                    ...
                    {requestData && requestData.length > 0 ? (
                        requestData.map((data, index) => (
                            <TableRow key={data._id}>
                                <TableCell className="w-2"><Input value={data._id} type="checkbox" checked={selectedRequests.some((item) => item._id === data._id)} onChange={() => handleSelectRequest(data)} /></TableCell>
