...
async function getAmountRanges({ startDate, endDate }) {
  const dateFilter = buildDateFilter(startDate, endDate);
...
async function getApprovalTrends({ startDate, endDate }) {
  const dateFilter = buildDateFilter(startDate, endDate);
...
async function getDepartmentTrends({ startDate, endDate }) {
  const dateFilter = buildDateFilter(startDate, endDate);
...
async function getProcessingTimeDistribution({ startDate, endDate }) {
  const dateFilter = buildDateFilter(startDate, endDate);
...
async function getHourlyPatterns({ startDate, endDate }) {
  const dateFilter = buildDateFilter(startDate, endDate);
...
async function getDashboardMetrics({ startDate, endDate }) {
  ...
  if (monthlyTrends.length >= 2) {
    ...
    if (prev > 0) {
      monthOverMonthGrowth = Number((((last - prev) / prev) * 100).toFixed(1));
    } else if (last > 0) {
